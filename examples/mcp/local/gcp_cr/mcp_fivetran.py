...
# HTTP Server Framework for Cloud Run
try:
    ...
    print("[SETUP] Installing FastAPI library...")
...
#!/usr/bin/env python3
"""
Fivetran API Server for Cloud Run (v3 - HTTP)

Architecture:
- FastAPI-based HTTP server for Google Cloud Run
- RESTful API endpoints wrapping Fivetran operations
- Direct execution without approval workflow
- Comprehensive error handling and retry logic

Usage:
    python mcp_fivetran_v3.py
    # Or with uvicorn: uvicorn mcp_fivetran_v3:app --host 0.0.0.0 --port 8080
...
app = FastAPI(
    title="Fivetran API Server",
    description="HTTP API server for Fivetran connector management",
    version="3.0.0"
)
...
# Configuration file path - single source of truth for all credentials (local development)
# Cloud Run deployments use environment variables instead
...
def _ensure_config_file_exists() -> None:
    ...
    Enterprise deployments should provision configuration explicitly via environment variables.
    """
    # Skip file check if using environment variables (Cloud Run)
    if os.getenv('FIVETRAN_API_KEY') and os.getenv('FIVETRAN_API_SECRET'):
        print(f"[INFO] Using environment variables for configuration (Cloud Run mode)")
        return
...
# Ensure configuration file exists at startup (only if not using env vars)
...
def _load_config(config_file_path: str = None) -> Dict:
    ...
    Priority:
    1. Environment variables (FIVETRAN_API_KEY, FIVETRAN_API_SECRET) - for Cloud Run
    2. JSON configuration file - for local development
    ...
    """
    # Check environment variables first (Cloud Run deployment)
    api_key = os.getenv('FIVETRAN_API_KEY')
    api_secret = os.getenv('FIVETRAN_API_SECRET')
    
    if api_key and api_secret:
        print(f"[INFO] Configuration loaded from environment variables (Cloud Run)")
        return {
            "fivetran_api_key": api_key,
            "fivetran_api_secret": api_secret
        }
    
    # Fall back to file-based configuration (local development)
...
async def root():
    ...
    return {"status": "healthy", "service": "Fivetran API Server v3", "version": "3.0.0"}
...
async def health():
    ...
    try:
        ...
        if result_json.get("success"):
            return JSONResponse(content={"status": "healthy", "fivetran_api": "connected"})
        else:
            return JSONResponse(status_code=503, content={"status": "unhealthy", "fivetran_api": "disconnected"})
    except Exception as e:
        return JSONResponse(status_code=503, content={"status": "unhealthy", "error": str(e)})

@app.get("/api/connectors")
async def api_list_connectors(group_id: TypingOptional[str] = Query(None)):
    """List all Fivetran connectors."""
    result = list_connectors(group_id)
    return JSONResponse(content=json.loads(result))

@app.get("/api/connectors/{connector_id}")
async def api_get_connector_status(connector_id: str):
    """Get connector status."""
    result = get_connector_status(connector_id)
    return JSONResponse(content=json.loads(result))

@app.post("/api/connectors/{connector_id}/pause")
async def api_pause_connector(connector_id: str):
    """Pause a connector."""
    result = pause_connector(connector_id)
    return JSONResponse(content=json.loads(result))

@app.post("/api/connectors/{connector_id}/resume")
async def api_resume_connector(connector_id: str):
    """Resume a connector."""
    result = resume_connector(connector_id)
    return JSONResponse(content=json.loads(result))

@app.get("/api/connectors/{connector_id}/check")
async def api_check_connector_status(connector_id: str):
    """Check connector health status."""
    result = check_connector_status(connector_id)
    return JSONResponse(content=json.loads(result))

@app.get("/api/connectors/{connector_id}/schema")
async def api_get_connector_schema(connector_id: str):
    """Get connector schema."""
    result = get_connector_schema(connector_id)
    return JSONResponse(content=json.loads(result))

@app.post("/api/connectors/{connector_id}/schema/reload")
async def api_reload_connector_schema(connector_id: str):
    """Reload connector schema."""
    result = reload_connector_schema(connector_id)
    return JSONResponse(content=json.loads(result))

class SchemaUpdateRequest(BaseModel):
    schema_config: Dict[str, Any]

@app.patch("/api/connectors/{connector_id}/schema")
async def api_update_connector_schema(connector_id: str, request: SchemaUpdateRequest):
    """Update connector schema."""
    result = update_connector_schema(connector_id, request.schema_config)
    return JSONResponse(content=json.loads(result))

@app.get("/api/connectors/{connector_id}/metrics")
async def api_get_connector_metrics(connector_id: str, days: int = Query(7)):
    """Get connector metrics."""
    result = get_connector_metrics(connector_id, days)
    return JSONResponse(content=json.loads(result))

@app.get("/api/metadata/connectors/{connector_type}")
async def api_get_connector_metadata(connector_type: str):
    """Get connector type metadata."""
    result = get_connector_metadata(connector_type)
    return JSONResponse(content=json.loads(result))
...
class CreateConnectorRequest(BaseModel):
    ...
    group_id: str
    config: Dict[str, Any]
    auth: TypingOptional[Dict[str, Any]] = None
    paused: bool = True
...
async def api_create_dynamic_connector(request: CreateConnectorRequest):
    """Create a new connector."""
    kwargs = {
        "paused": request.paused,
        "sync_frequency": request.sync_frequency,
        "trust_certificates": request.trust_certificates,
        "trust_fingerprints": request.trust_fingerprints
    }
    result = create_dynamic_connector(
...
class MigrateConnectorRequest(BaseModel):
    target_group_id: str
    new_schema_name: TypingOptional[str] = None
    new_table_name: TypingOptional[str] = None

@app.post("/api/connectors/{connector_id}/migrate")
async def api_migrate_connector(connector_id: str, request: MigrateConnectorRequest):
    """Migrate connector to different destination."""
    result = migrate_connector(
        connector_id,
...
@app.patch("/api/connections/{connection_id}/sync-frequency")
async def api_modify_sync_frequency(connection_id: str, sync_frequency: int = Query(...)):
    """Modify connection sync frequency."""
    result = modify_sync_frequency(connection_id, sync_frequency)
    return JSONResponse(content=json.loads(result))

@app.get("/api/health-check")
async def api_health_check_all_connectors(group_id: TypingOptional[str] = Query(None)):
    """Health check all connectors."""
    result = health_check_all_connectors(group_id)
    return JSONResponse(content=json.loads(result))

@app.get("/api/usage-report")
async def api_get_connector_usage_report(group_id: TypingOptional[str] = Query(None), days: int = Query(30)):
    """Get connector usage report."""
    result = get_connector_usage_report(group_id, days)
    return JSONResponse(content=json.loads(result))

@app.get("/api/destinations")
async def api_get_simple_destinations():
    """Get simple destinations list."""
    result = get_simple_destinations()
    return JSONResponse(content=json.loads(result))

@app.get("/api/test-connection")
async def api_test_connection():
    """Test Fivetran API connection."""
    result = test_connection()
    return JSONResponse(content=json.loads(result))
...
if __name__ == "__main__":
    ...
    print(f"[INFO] Starting Fivetran API Server (v3 - HTTP)...")
