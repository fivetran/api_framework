...
from concurrent.futures import ThreadPoolExecutor, as_completed  # Parallel table processing
...
DEFAULT_START_DATE = "2023-10-01T00:00:00.000Z"
DEFAULT_BATCH_DELAY_MS = 100
DEFAULT_MAX_RETRIES = 3
DEFAULT_RETRY_DELAY_SEC = 5
CHECKPOINT_INTERVAL = 1000  # Checkpoint every x records
CONNECTION_TIMEOUT_HOURS = 3  # Reconnect after x hours
...
def update(configuration: dict, state: dict):
    ...
    # Optional parallelization via custom_threads (capped at 4 and resource-aware)
    requested_threads_raw = get_config_value(configuration, "custom_threads", "0")
    try:
        requested_threads = max(0, int(str(requested_threads_raw)))
    except Exception:
        requested_threads = 0

    max_workers = 1
    if requested_threads > 0:
        max_workers = min(requested_threads, 4)
        if PSUTIL_AVAILABLE and enable_resource_monitoring:
            status = monitor_resources()
            if status.get('status') == 'active':
                # Use the same reduction logic as resource monitor
                _, reduced = should_reduce_threads(status.get('cpu_percent', 0.0), max_workers)
                max_workers = max(1, min(max_workers, reduced))
        log.info(f"Parallel replication enabled: {max_workers} thread(s) (requested {requested_threads}, cap 4)")
    else:
        log.info("Parallel replication disabled (custom_threads not set or 0)")

    results_lock = threading.Lock()

    # Helper to process a single table; each worker uses its own connection manager
    def _process_table(table: str, category: str, row_count: int):
    ...
    if max_workers > 1 and total_tables > 1:
        # Run tables in the pre-sorted order, but allow concurrency up to max_workers
        with ThreadPoolExecutor(max_workers=max_workers, thread_name_prefix="sync") as executor:
            futures = [executor.submit(_process_table, table, category, row_count) for table, category, row_count in categorized_tables]
            for _ in as_completed(futures):
                pass
    else:
        # Sequential fallback (original behavior, but using the same helper for consistent logging)
        for table, category, row_count in categorized_tables:
            _process_table(table, category, row_count)
