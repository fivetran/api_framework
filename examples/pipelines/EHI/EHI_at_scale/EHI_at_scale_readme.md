...
- Resource Monitoring: Real-time system resource monitoring with automatic parameter adjustment (requires psutil library)
...
- `MSSQL_CERT_SERVER`: SQL Server certificate server hostname (for SSL certificate generation)
- `MSSQL_PORT`: SQL Server port number (typically 1433)
- `MSSQL_DATABASE`: Name of the database to connect to
- `MSSQL_USER`: Database username for authentication
- `MSSQL_PASSWORD`: Database password for authentication
...
- `cert`: SSL certificate content (PEM format) or path to certificate file. If not provided, certificate will be generated automatically.
- `threads`: Number of threads for parallel processing (default: adaptive based on table size, max 4)
- `max_queue_size`: Maximum size of the processing queue (default: adaptive based on table size)
- `max_retries`: Maximum number of retry attempts for failed operations (default: 5)
- `retry_sleep_seconds`: Base delay in seconds for exponential backoff retry logic (default: 5)
...
1. Install Python dependencies required by the connector:
```bash
...
The connector can be tested locally using the Fivetran debug command:
...
- Adaptive Processing: Processing parameters automatically adjust based on table size
